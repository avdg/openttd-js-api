		...
		describe('Type Table', function () {
			...
			describe('Send', function () {
				...
				it('Should be able to create floatLE buffers', function() {
					var buffer = generics.typeTableSend['floatle'](.3);

					assert.equal(buffer.length, 4);
					assert.equal(buffer.readFloatLE(0) > .299, true);
					assert.equal(buffer.readFloatLE(0) < .301, true);
				});

				it('Should be able to create floatBE buffers',  function() {
					var buffer = generics.typeTableSend['floatbe'](.3);

					assert.equal(buffer.length, 4);
					assert.equal(buffer.readFloatBE(0) > .299, true);
					assert.equal(buffer.readFloatBE(0) < .301, true);
				});

				it('Should be able to create doubleLE buffers', function() {
					var buffer = generics.typeTableSend['doublele'](.3);

					assert.equal(buffer.length, 8);
					assert.equal(buffer.readDoubleLE(0) > .29999, true);
					assert.equal(buffer.readDoubleLE(0) < .30001, true);
				});

				it('Should be able to create doubleBE buffers', function() {
					var buffer = generics.typeTableSend['doublebe'](.3);

					assert.equal(buffer.length, 8);
					assert.equal(buffer.readDoubleBE(0) > .29999, true);
					assert.equal(buffer.readDoubleBE(0) < .30001, true);
				});
