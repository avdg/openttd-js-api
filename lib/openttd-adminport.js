...
module.exports = tcpPacketListener = (function() {
	...
	function tcpPacketListener(settings) {
		...
		this.client.on("error", this.defaultOnError);
	}

	tcpPacketListener.prototype.defaultOnConnect = function() {
		self._log("Client connected");
		self.emitter.emit.apply(self.emitter, ['connect'].concat(arguments));
		self.sendPacket(
	...
	tcpPacketListener.prototype.defaultOnEnd = function() {
		...
		self.emitter.emit.apply(self.emitter, ['end'].concat(arguments))
	};

	tcpPacketListener.prototype.defaultOnError = function() {
		self.emitter.emit.apply(self.emitter, ['error'].concat(arguments))
	}
	...
	tcpPacketListener.prototype.sendPacket = function(type, data) {
		...
		if (!(type in spec.AdminPacketDescription))
			throw "No package of type " + type + " found"
